...
class CalibrationTab:
    ...
    def calibration_convert(self):
        ...
        Y_plateaus_cal = self.fitpoints_tab.Y_plateaus_cal
        ...
        Y_plateaus_cal_conv = np.log10(self.convert_rho_to_N(np.power(10., Y_plateaus_cal)))
        Y_cal_conv = np.log10(self.convert_rho_to_N(np.power(10., Y_cal)))
        print(f"PyQt - calibration_convert: Y_plateaus_cal_conv min={np.min(Y_plateaus_cal_conv):.3f}, max={np.max(Y_plateaus_cal_conv):.3f}")
        ...
        ax.plot(np.power(10., self.fitpoints_dat_opt), np.power(10., Y_plateaus_cal_conv), 
                color='r', ls='-', label="calibration (optimized)", zorder=1)
        self.redraw_calibration_curve_init(ax, X_cal, Y_cal_conv, Y_plateaus_dat, Y_plateaus_cal_conv, mode="Calibration")
        ax.legend(loc='best', fontsize=10)
        ...
        self.Y_plateaus_cal_conv = Y_plateaus_cal_conv
        ...
        try:
            ...
            #self.ui.export_excel_pushButton.setStyleSheet("background-color: yellow; color: black")
            #self.ui.export_excel_pushButton.setEnabled(True)
            #self.ui.export_png_pushButton.setStyleSheet("background-color: yellow; color: black")
            #self.ui.export_png_pushButton.setEnabled(True)
            #self.ui.export_gwy_pushButton.setStyleSheet("background-color: yellow; color: black")
            #self.ui.export_gwy_pushButton.setEnabled(True)
    ...
    def export_database(self):
        ...
        quality = getattr(self.alignment_tab, 'quals', [0])[0]  # Adjust based on actual quals storage
        ...
        # Update button state
        try:
            self.ui.export_database_pushButton.setStyleSheet("background-color: green; color: black")
